...
function openModal3(aid) {
    ...
    $.ajax({
        url: 'http://localhost/Interlearn/public/instructor/course/getAnnouncement?aid='+aid,
        type: 'GET',
                ...
                if(response[i].aid == aid){
                    ...
                    for (i = 0; i < close.length; i++) {
                      close[i].onclick = function() {
                        ...
                        $.ajax({
                            method:"POST",
                            url : 'http://localhost/Interlearn/public/instructor/course/editAnnouncementFile?aid='+aid,
                            data:{'aid' : aid},
                            success:function(response){
                              console.log(response);
                            },
                    ...
                    submitAnnouncement.addEventListener("click", function(event) {
                      ...
                      console.log(newAttachment[0]);

                      formData.append('aid', aid);
                      formData.append('title', title);
                      formData.append('content', content);
                      formData.append('file_name', fileName);
                      if(newAttachment[0] != 'undefined'){
                        console.log("hi");
                      ...
                      $.ajax({
                        method:"POST",
                        url : 'http://localhost/Interlearn/public/instructor/course/submitEditAnnouncement',
                        data:formData,
                        processData: false,
                        contentType: false,
                        success:function(response){
