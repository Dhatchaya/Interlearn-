...
const p_mobile = document.querySelector('#parent_mobile');
const p_name = document.querySelector('#parent_name');
const p_email = document.querySelector('#parent_email');
...
const error14 = document.querySelector('#errorSpace14');
const pname = document.querySelector ('#change-pname');
const pemail = document.querySelector ('#change-pemail');
const pmobile = document.querySelector ('#change-pmobile');
...
pname.addEventListener('click', () => {
    ...
    p_name.removeAttribute  ('readonly');
    p_name.focus();
});
pemail.addEventListener('click', () => {
    pemail.style.display = 'none';
    p_email.removeAttribute  ('readonly');
    p_email.focus();
});
pmobile.addEventListener('click', () => {
    pmobile.style.display = 'none';
    p_mobile.removeAttribute  ('readonly');
    p_mobile.focus();
});
...
saveBtn.addEventListener('click', (e) => {
    ...
    if(p_mobile.value!==""&&isNaN(p_mobile.value)){
        error14.innerHTML = "Not a valid contact number";
        $isAllFilled = false;
    }
    else if(p_mobile.value!==""&&p_mobile.value.length < 10){
        error14.innerHTML = "Contact number must be 10 digits";

        $isAllFilled = false;
    }

    p_mobile.addEventListener('input', function(event) {
        error11.innerHTML = "";
    });
    ...
    oldPW.addEventListener('input', function(event) {
        error10.innerHTML = "";
    });
    ...
    if ($isAllFilled) {
        ...
        fetch('/Interlearn/public/Student/profile/editUser', {
            method: 'POST',
